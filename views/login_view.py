...
class AuthController:
    ...
    def es_operador(self) -> bool:
        """Devuelve True si el usuario activo es Operador de Inventario o superior."""
        return "OPERADOR" in self.rol_actual.upper() or self.es_administrador()

    # PARCHE QA: Control de Acceso Basado en Roles (RBAC)-

    # Matriz de permisos: id de módulo permitido por cada rol del sistema.
    PERMISOS_POR_ROL = {
        "ADMINISTRADOR EJECUTIVO": [
            "equipos", "prestamos", "asignacion", "alarmas", "usuarios", "historial"
        ],
        "OPERADOR DE INVENTARIO": [
            "equipos", "prestamos", "asignacion", "alarmas"
        ],
        "PROPIETARIO": [
            "prestamos"
        ],
    ...
    def tiene_acceso_modulo(self, id_modulo: str) -> bool:
        """
        Indica si el rol de la sesión activa tiene permiso para acceder
        al módulo indicado (por ejemplo: "equipos", "usuarios", "historial").

        Si el rol no coincide con ninguno conocido, se niega el acceso
        por defecto (principio de menor privilegio).
        """
        return id_modulo in self.modulos_permitidos()

    def modulos_permitidos(self) -> list[str]:
        """
        Devuelve la lista de módulos visibles para el rol de la sesión activa.
        
        """
        rol = self.rol_actual.strip().upper()
        permitidos = set()
        for nombre_rol, modulos in self.PERMISOS_POR_ROL.items():
            if nombre_rol in rol:
                permitidos.update(modulos)
