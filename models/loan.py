...
def obtener_todos(texto_busqueda: str = "", filtro_estado: str = "ALL") -> list:
    ...
    if filtro_estado.upper() != "ALL":
        ...
        # Vista activa de trazabilidad: los préstamos ya cerrados (DEVUELTO)
        # no deben listarse aquí. Para auditoría de cerrados existe el
        # módulo de Historial, que consulta su propio modelo.
        condiciones.append("UPPER(estado) != 'DEVUELTO'")
...
def registrar_prestamo(id_activo: int, prestatario: str,
    ...
    # --- Insertar el préstamo (PARCHE QA: ahora incluye ubicación) ---
    cursor.execute("""
        INSERT INTO prestamos (equipo, serial, prestatario, fecha_p, fecha_d, estado, ubicacion)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (
